...
{ id: 'memory-sonidos', href: '/memory-sonidos', label: 'Memory de Sonidos', desc: 'Escucha la secuencia de sonidos y repítela en el mismo orden.' },
  ...
  { id: 'gin-rummy', href: '/gin-rummy', label: 'Gin Rummy', desc: 'Juego de cartas clásico. Forma combinaciones (tríos o escaleras) con tus 10 cartas y llama cuando tengas 10 puntos sueltos o menos. Roba con M o D, descarta con 1-0 y R.' },
  { id: 'poker', href: '/poker', label: "Póker Texas Hold'em", desc: "Texas Hold'em contra la IA. Recibes 2 cartas y compites con 5 cartas comunitarias. Iguala, pasa o sube en cada ronda. F=retirarte, C=igualar/pasar, R=subir. Empieza con 1000 fichas." },
  { id: 'truco', href: '/truco', label: 'Truco', desc: 'Truco argentino 1 contra 1. Mazo español de 40 cartas, 3 bazas por mano. Canta Envido (E) antes de la primera baza y Truco (T) en cualquier momento. Primero en llegar a 15 puntos gana.' },
  { id: 'parchis', href: '/parchis', label: 'Parchís', desc: 'Parchís 1 contra 1. 4 fichas cada jugador (rojas tú, azules la IA). Lanza el dado con R, elige ficha con 1-4. Saca un 5 para salir de casa. Seis y capturas dan turno extra. Gana quien meta las 4 fichas primero.' },
...
// Metadatos SEO de la página de un juego (title, description y canonical
// propios). Las páginas de juego son componentes cliente y no pueden exportar
// metadata, así que cada carpeta tiene un layout.tsx mínimo que llama a esto.
export function gameMetadata(slug: string): Metadata {
  ...
  return {
    title: `${game.label} accesible para ciegos`,
    description: `${game.label}, juego gratis y accesible para personas ciegas, con teclado y lector de pantalla. ${game.desc}`,
    alternates: { canonical: game.href },
  }
}

// Slugs de las rutas de juego de Next.js (excluye los HTML estáticos de public/games).
export const GAME_SLUGS = GAMES.filter((g) => !g.href.startsWith('/games/')).map((g) => g.href.slice(1))
